...
async def help(ctx: lightbulb.Context):
    """
    Displays the help information for the commands.

    Args:
        ctx (lightbulb.Context): The context object representing the invocation context.
...
async def on_component_interaction(event: hikari.InteractionCreateEvent) -> None:
    ...
    if interaction_info[0] == "search":
        ...
        embed = hikari.Embed(title=title_str, description=output_str)
        if label == "Back":
...
@lightbulb.option(
    "top_x_results",
    "Specifies the top x results, default is 100",
    required=False,
    default=100,
)
